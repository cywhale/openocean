...
import HeightmapTerrainData from 'cesium/Source/Core/HeightmapTerrainData';
...
import getImagePixels from 'cesium/Source/Core/getImagePixels';
import when from 'cesium/Source/ThirdParty/when';

//(function() {
    var OGCHelper = {};
    const loadXML=loadXML===undefined?Resource.fetchXML:loadXML;
    const loadArrayBuffer=loadArrayBuffer===undefined?Resource.fetchArrayBuffer:loadArrayBuffer;
    const loadImage=loadImage===undefined?Resource.fetchImage:loadImage;
     ...
     */
    OGCHelper.parser = function(description=defaultValue.EMPTY_OBJECT) {
        let resultat;
    ...
    OGCHelper.WMSParser.generate = function(description=defaultValue.EMPTY_OBJECT) {
        let resultat;
        if (defined(description.url)) {
            ...
            resultat = loadXML(urlGetCapabilities).then(xml => {
              OGCHelper.WMSParser.getMetaDatafromXML(xml, description)
    ...
    OGCHelper.WMSParser.getMetaDatafromXML = function(xml, description) {
        ...
        let maxLevel = defaultValue(description.maxLevel, 11);
        ...
        if (!defined(resultat.formatImage)) {
            for (let j = 0; j < nodeFormats.length && !defined(resultat.formatArray); j++) {
        ...
        for (let j = 0; j < nodeFormats.length && !defined(resultat.formatImage); j++) {
        ...
        console.log("Debug in GeoserverTerrainProvider WMS with layerNodes[0]: ", layerNodes[0]);
        let layName = layerName;
        let laySubdir = '';
        let layidx = layerName.indexOf(':');
        if (layidx >= 1) { //cannot be first character, for "datadir:layerName" example
          laySubdir = layerName.substring(0, layidx);
          layName = layerName.substring(layidx+1);
        }

        let layerNode;
        for (let m = 0; m < layerNodes.length && !defined(layerNode); m++) {
            if (layerNodes[m].querySelector("Name").textContent === layName) {
        ...
        if (defined(layerNode) && defined(version)) {
            ...
            for (let n = 0; n < OGCHelper.CRS.length && !found; n++) {
                ...
                if (nodeBBox !== null) {
                    ...
                    let rectReference = new Rectangle(west, south, east, north);
            ...
            for (let q = 0; q < tileSets.length && !out; q++) {
                let isGoodSRS = tileSets[q].querySelector("BoundingBox[SRS='" +
                    CRS + "'],BoundingBox[CRS='" +
                    CRS + "']") !== null;
                let isGoodLayer = tileSets[q].querySelector("Layers").textContent === layName;

                console.log("Debug in GeoserverTerrainProvider WMS with tileSets[q]]: ", tileSets[q].querySelector("Layers").textContent);
            ...
            console.log("Debug in GeoserverTerrainProvider WMS with found: ", found);
        ...
        console.log("Debug in GeoserverTerrainProvider WMS with ready: ", resultat.ready);
        ...
        if (resultat.ready) {
            ...
            console.log("Debug in GeoserverTerrainProvider WMS with urltemplate: ", URLtemplate);
    ...
    OGCHelper.TMSParser.generate = function(description=defaultValue.EMPTY_OBJECT) {
        let resultat;
        if (defined(description.url)) {
    ...
    OGCHelper.TMSParser.parseXML = function(xml, description) {
        ...
        if (xml.querySelector("TileMapService") != null) {
            ...
            let promise = when.all(promises).then(function(tabResult) {
                let retour;
                for (let i = 0; i < tabResult.length && !defined(retour); i++) {
    ...
    OGCHelper.TMSParser.getMetaDatafromXML = function(xml, description) {
        ...
        if (!defined(resultat.tileSets) || !defined(resultat.formatImage) || !defined(resultat.tilingScheme)) {
            ...
            let limites = new Rectangle(minx, miny, maxx, maxy);
            resultat.getTileDataAvailable = function(x, y, level) {
    ...
    OGCHelper.WMTSParser.generate = function(description=defaultValue.EMPTY_OBJECT) {
        let resultat;
        if (defined(description.url)) {
            let urlofServer = description.url;
            let index = urlofServer.lastIndexOf("?");
    ...
    OGCHelper.WMTSParser.getMetaDatafromXML = function(xml, description) {
        ...
        resultat.heightMapWidth = defaultValue(description.heightMapWidth, 65);
        resultat.heightMapHeight = defaultValue(description.heightMapHeight, resultat.heightMapWidth);
        let maxLevel = defaultValue(description.maxLevel, 12);
        let proxy = description.proxy;
        let styleName = description.styleName;
        resultat.hasStyledImage = defaultValue(description.hasStyledImage, typeof(description.styleName) === "string");
        resultat.waterMask = defaultValue(description.waterMask, false);
        ...
        let layerNode;
        for (let i = 0; i < nodeIdentifiers.length && !defined(layerNode); i++) {
        ...
        for (let a = 0; a < listTileMatrixSetLinkNode.length && !resultat.ready; a++) {
            ...
            for (let i = 0; i < nodeMatrixSetIds.length && !defined(tileMatrixSetNode); i++) {
            ...
            for (let n = 0; n < OGCHelper.CRS.length && !defined(CRSSelected); n++) {
...
//  let GeoserverTerrainProvider = function GeoserverTerrainProvider(description) {
    export default function GeoserverTerrainProvider(description) {
        ...
        let promise = this.OGCHelper.parser(description);
        let result = TerrainParser(promise, this);
        console.log("Terrain provider result: ", result)
    };
    /**
     *
    ...
    GeoserverTerrainProvider.imageToHeightmapTerrainData = function(image, limitations, size, hasWaterMask, childrenMask, hasStyledImage) {
        ...
        let dataPixels = getImagePixels(image, size.width, size.height);
